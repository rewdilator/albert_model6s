...
4-door EV sedan (pearl white) wearing the **Albert** brand, with independently opening doors, frunk, trunk and a detailed CCS2 charge port: a faceted 3D **A+** logo (dark chrome, chrome edge, black border) on the nose and steering wheel, a large **ALBERT** wordmark on the trunk, chrome **A+** on the wheel caps, embroidered **A+** headrests, **ALBERT** door-sill plates and a full Albert touchscreen UI. Full technical details, dimensions, Unity/RCC setup and material/texture tables: **`EV_Sedan_TechnicalSpec.md`**.
...
| `brand/` | Albert logo files for your game UI: `A+` emblem, `ALBERT` wordmark and stacked lockup as SVG + transparent PNG (white / black), a 3D render of the badge and a transparent car render |
...
Triangles: LOD0 475,341 / LOD1 169,211 / LOD2 56,559.
Photo-scanned textures are CC0 (ambientCG.com).

## Rebuilding
All scripts run with Blender 4.5 (`blender -b -P <script>`) or with the `bpy` Python module (`pip install bpy==4.5.*`, Python 3.11). The logo geometry needs Shapely (`pip install shapely`; inside Blender: `<blender python> -m pip install shapely`):

1. `python3 tools/make_brand_assets.py` - writes `brand/` and draws the touchscreen UI into `textures/ev_sedan_screen_d.png` (needs Pillow and Shapely: `pip install pillow shapely`).
...
7. `python3 tools/render_previews.py [shot ...]` - re-renders `preview/` with Cycles; `python3 tools/render_logo_3d.py` renders `brand/albert_logo_3d.png`.
